...
const AddPatient = () => {
  ...
  const checkForDuplicate = debounce(async (patientName, nativeLanguage) => {
    ...
    try {
      ...
      setDuplicateWarning(
        duplicate
          ? `A patient with the name "${patientName}" and language "${nativeLanguage}" already exists.`
          : null
      );
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        ...
        setDuplicateWarning(
          error.response.data.message ||
            `A patient with the name "${trimmedName}" and language "${trimmedLanguage}" already exists.`
        );
        ...
        setError(
          error.response?.data?.message ||
            'Failed to create patient/therapy plan. Please try again.'
        );
      }
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 bg-[#B2D1CF] bg-opacity-[0.80] backdrop-blur-md rounded-lg shadow-lg p-10 w-[800px]">
        ...
        {duplicateWarning && (
          <div className="mb-4 p-2 bg-yellow-100 text-yellow-700 rounded text-center">
            {duplicateWarning}
            <button
              onClick={() => {
                setDuplicateWarning(null); // Clear warning and retry
                handleSubmit({ preventDefault: () => {} }); // Resubmit
              }}
